...
watch:
  - A GitHub push updates `refs/heads/master`.
routines:
  - Run `bun .agents/daemons/pr-merge-conflict-repair/scripts/find-conflicted-pulls.ts --repo charlie-labs/daemons` to find open non-draft PRs with mergeability state `CONFLICTING`.
...
For this repository, `master` is the default branch. Default-branch pushes are the survey trigger, not proof that every conflict was caused by the default branch. The discovery script may return conflicted non-draft PRs targeting any base branch; repair each PR against its own current base.
