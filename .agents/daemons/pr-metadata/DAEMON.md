...
The configured issue ID pattern is a GitHub issue reference in this repository: `#` followed by a decimal issue number (for example, `#18`). PR title suffixes must use the parenthesized form, such as `(#18)`; PR body issue-reference lines must use the bare issue reference, such as `Resolves #18` or `Refs #18`.
...
The PR title should end with exactly one parenthesized GitHub issue token, such as `(#18)`.

When the primary issue is clear, patch only the trailing parenthesized issue suffix:
...
- `Resolves #18`-style lines when the issue appears to be resolved by the PR.
- `Refs #18`-style lines when the issue is related but not clearly resolved.
