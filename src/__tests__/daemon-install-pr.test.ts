...
describe('daemon install PR API', () => {
  test('creates a deterministic branch, atomic commit, PR, and hidden marker without raw adaptation values', async () => {
    ...
    expect(pullBody).toContain('charlie-daemon-install-v1');
    expect(pullBody).toContain('`required_value`');
    expect(pullBody).not.toContain('secret-target');
