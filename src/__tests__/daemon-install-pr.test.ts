...
${result.markerText}`);
  ...
  test.each([
    ...
    ['tampered registry repo', 'charlie-daemon-install-v2', { registryRepo: 'attacker/repo' }],
    ['tampered registry ref', 'charlie-daemon-install-v2', { registryRef: 'main' }],
    ['mismatched registry slug', 'charlie-daemon-install-v2', { registrySlug: 'different-daemon' }],
    ['unpinned registry source ref', 'charlie-daemon-install-v2', { sourceRef: 'main' }],
    ['uppercase registry source ref', 'charlie-daemon-install-v2', { sourceRef: 'A'.repeat(40) }],
